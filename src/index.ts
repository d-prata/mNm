...
let inputEmailLogin = document.querySelector(
  "#input-email"
) as HTMLInputElement;
let inputSenhaLogin = document.querySelector(
  "#input-senha"
) as HTMLInputElement;
let btnCriar = document.querySelector("#btn-criar") as HTMLButtonElement;
let btnVoltarLogin = document.querySelector(
  "#btn-voltar-login"
) as HTMLButtonElement;
let divPrincipalLogin = document.querySelector(
  "#div-principal-login"
) as HTMLDivElement;
let divPrincipalSignup = document.querySelector(
  "#div-principal-signup"
) as HTMLDivElement;
...
let labelInputEmailSignup = document.querySelector(
  "#label-input-email"
) as HTMLLabelElement;
let inputEmailSignup = document.querySelector(
  "#input-signup-email"
) as HTMLInputElement;
let validEmailSignup: boolean = false;

let labelInputSenhaSignup = document.querySelector(
  "#label-input-senha"
) as HTMLLabelElement;
let inputSenhaSignup = document.querySelector(
  "#input-signup-senha"
) as HTMLInputElement;
let validSenhaSignup: boolean = false;

let labelInputRepeteSenhaSignup = document.querySelector(
  "#label-input-repete-senha"
) as HTMLLabelElement;
let inputRepeteSenhaSignup = document.querySelector(
  "#input-signup-repete-senha"
) as HTMLInputElement;
...
// ALERTAS
let espacoAlertaIndex = document.getElementById(
  "espaco-alerta-index"
) as HTMLDivElement;
let espacoAlertaSignupIndex = document.getElementById(
  "espaco-alerta-signup-index"
) as HTMLDivElement;
let corpoAlertaIndex: HTMLDivElement = document.createElement("div");

function mostrarAlertaIndex(
  mensagem: string,
  tipo: string,
  espaco: HTMLDivElement
) {
...
function validaUsuarioLogin() {
  ...
  let usuarioExistente: boolean = usuarios.some(
    (usuario) => usuario.email === inputEmailLogin.value
  );

  if (!usuarioExistente) {
    mostrarAlertaIndex(
      "Usuário não cadastrado! Primeiro crie uma conta.",
      "warning",
      espacoAlertaIndex
    );
  }
...
function verificaEmailSignup() {
  if (inputEmailSignup.value.length < 10) {
    ...
    labelInputEmailSignup.innerHTML =
      "e-mail: * insira no mínimo 10 caracteres";
...
function verificaSenhaSignup() {
  ...
  if (inputSenhaSignup.value.length < 8) {
    labelInputSenhaSignup.style.color = "red";
    labelInputSenhaSignup.innerHTML = "senha: * insira no mínimo 8 caracteres";
    validSenhaSignup = false;
  } else if (senhaValida === null) {
    labelInputSenhaSignup.innerHTML =
      "senha: * senha deve conter uma letra maiúscula e um caracter especial";
    labelInputSenhaSignup.style.color = "red";
    validSenhaSignup = false;
  } else {
...
function verificaRepeteSenhaSignup() {
  if (inputRepeteSenhaSignup.value !== inputSenhaSignup.value) {
    ...
    labelInputRepeteSenhaSignup.innerHTML =
      "repete senha: * a senha digitada não corresponde";
...
function verificaCampos() {
  ...
  let usuarioExistente: boolean = JSON.parse(
    localStorage.getItem("usuarios") || "[]"
  ).some(
    (usuario: Usuario) =>
      usuario.email === JSON.stringify(inputEmailSignup.value)
  );
  ...
  if (
    ...
    mostrarAlertaIndex(
      "Você deve preencher todos os campos!",
      "warning",
      espacoAlertaSignupIndex
    );
    retorno = false;
  } else if (
    !validEmailSignup ||
    !validSenhaSignup ||
    !validRepeteSenhaSignup
  ) {
    mostrarAlertaIndex(
      "Você deve preencher todos os campos!",
      "warning",
...
function criarRegistroUsuario() {
  let dadosStorage: Usuario[] = JSON.parse(
