...
let divPrincipalPainel = document.querySelector(
  "#div-principal-painel"
) as HTMLDivElement;
...
// ALERTAS
let espacoAlertaRecados = document.getElementById(
  "espaco-alerta-recados"
) as HTMLDivElement;
...
function validaIdUnico() {
  ...
  let existe: boolean = recadosDoUsuarioLogado.some(
    (recado) => recado.id == id
  );

  if (existe) {
    mostrarAlertaRecados(
      "O ID deve ser único, você já tem um recado com este ID.",
      "warning"
    );
    inputId.value = "";
    inputId.focus();
...
function validaCamposPreenchidos() {
  ...
  if (
    ...
    inputDescricao.value.length === 0 ||
    inputDetalhamento.value.length === 0
  ) {
...
function salvarRecado() {
  ...
  if (edicao) {
    indiceRecadoEdicao = recadosDoUsuarioLogado.findIndex(
      (recado) => (recado.id = id)
    );
...
function pegarDadosStorage() {
  ...
  indiceDoUsuarioLogado = listaUsuarios.findIndex(
...
function apagarRecado(id: number) {
  let indiceEncontrado = recadosDoUsuarioLogado.findIndex(
    (recado) => recado.id === id
  );
...
function editarRecado(id: number) {
  let recado: Recado | undefined = recadosDoUsuarioLogado.find(
    (recado) => recado.id === id
  );
